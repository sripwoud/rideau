...
export class RootsService {
  ...
  async find(findRootDto: FindRootDto) {
    // TODO handle error, null is devil
    return this.supabase.from('roots').select().eq('group_id', findRootDto).single() // group_id is a unique key so there can only be one
  }
}
